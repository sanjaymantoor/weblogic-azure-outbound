...
usage() { echo "Usage: $0 WLS_GITHUB_REPOSITORY GIT_USER GIT_TOKEN DEPLOY_OFFER GIT_USER_EMAIL RESOURCE_GROUP_NAME \
  ...
  WLS_GITHUB_REPOSITORY            : WebLogic azure repository. https://github.com/<user>/weblogic-azure-pack-unpack.git
...
{
	cd $WORK_DIRECTORY
	if [ -d weblogic-azure-pack-unpack ];
	then
		rm -rf weblogic-azure-pack-unpack
	fi
...
case "${DEPLOY_OFFER}" in 
	"admin")
			if [[ -z $RAW_MAIN_TEMPLATE_URL ]]; then
				RAW_MAIN_TEMPLATE_URL="https://raw.githubusercontent.com/$GIT_USER/weblogic-azure-pack-unpack/$TEST_BRANCH_NAME/weblogic-azure-vm/arm-oraclelinux-wls-admin/src/main/arm/mainTemplate.json"
			fi
	...
	"cluster")
			if [[ -z $RAW_MAIN_TEMPLATE_URL ]]; then
				RAW_MAIN_TEMPLATE_URL="https://raw.githubusercontent.com/$GIT_USER/weblogic-azure-pack-unpack/$TEST_BRANCH_NAME/weblogic-azure-vm/arm-oraclelinux-wls-cluster/arm-oraclelinux-wls-cluster/src/main/arm/mainTemplate.json"
			fi
	...
	"dynamic")
			if [[ -z $RAW_MAIN_TEMPLATE_URL ]]; then
				RAW_MAIN_TEMPLATE_URL="https://raw.githubusercontent.com/$GIT_USER/weblogic-azure-pack-unpack/$TEST_BRANCH_NAME/weblogic-azure-vm/arm-oraclelinux-wls-dynamic-cluster/arm-oraclelinux-wls-dynamic-cluster/src/main/arm/mainTemplate.json"
			fi
			deployClusterOffer $PARAMETERS_FILE_PATH $RESOURCE_GROUP_NAME $RAW_MAIN_TEMPLATE_URL $LOCATION $SKU_VERSION $ADDITIONAL_DEPLOYMENT_PARAMETERS
			TEST_CLUSTER_OFFER_SCRIPT="runClusterOfferTest.sh"
			testOffer $WLS_TEST_GIT_REPOSITORY $TEST_RAW_MAIN_TEMPLATE_URL $RESOURCE_GROUP_NAME $RUN_TESTS $TEST_INPUT_FILE $TEST_CLUSTER_OFFER_SCRIPT $LOCATION $TEST_ARTIFACT_LOCATION $ADDITIONAL_TEST_PARAMETERS
